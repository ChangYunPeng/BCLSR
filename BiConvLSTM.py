...
class BiConvLSTM(nn.Module):
    ...
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers,
                 bias=True, return_all_layers=False, return_fl = True):
        ...
        #                                     bias=self.bias))
        # self.forward_cell_list = nn.ModuleList(cell_list)
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(BiConvLSTMCell(input_size=(self.height, self.width),
                                            ...
                                            kernel_size=self.kernel_size[i],
                                            bias=self.bias))
        self.backward_cell_list = nn.ModuleList(cell_list)

        
        cell_list = []
        for i in range(0, self.num_layers):
            print( self.kernel_size[i])
            cell_list.append(nn.Conv2d(in_channels= 2 * self.hidden_dim[i],
                                     out_channels=self.hidden_dim[i],
                                    #  kernel_size=self.kernel_size[i],
                                    #  padding=(self.kernel_size[i][0]//2,self.kernel_size[i][1]//2),
                                     kernel_size=[3,3],
                                     padding=(1,1),
                                     bias=self.bias) )
        self.conv_concat = nn.ModuleList(cell_list)
    ...
    def forward(self, input_tensor):
        ...
        cur_backward_input = input_tensor
        cur_forward_input = input_tensor
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                hb, cb = self.backward_cell_list[layer_idx](input_tensor=cur_layer_input[:, seq_len - t - 1, :, :, :], cur_state=[hb, cb])
                backward_states.append(hb)

            hf, cf = hidden_state[layer_idx]
            for t in range(seq_len):
                hf, cf = self.backward_cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :], cur_state=[hf, cf])
                # hf, cf = self.forward_cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :], cur_state=[hf, cf])
                forward_states.append(hf)

            # # forward backward tensor
            # cur_backward_input = torch.stack(backward_states, dim=1)
            # cur_forward_input = torch.stack(forward_states, dim=1)
        # hidden
            
            # import pdb; pdb.set_trace()
            for t in range(seq_len):

                h = self.conv_concat[layer_idx](torch.cat((forward_states[t], backward_states[seq_len - t - 1]), dim=1))
                # h = torch.cat((forward_states[t], backward_states[seq_len - t - 1]), dim=1)
        ...
        if self.return_fl:
            return torch.cat((layer_output_list[:,0,:,:,:],layer_output_list[:,-1,:,:,:]), dim=1) 
